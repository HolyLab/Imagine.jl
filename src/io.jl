...
function output_signals(sigs; dev = DEFAULT_DEVICE, trigger_terminal = "disabled", run_locally = false, clock = "")
    ...
    if !isempty(sigsa)
        ...
        if !run_locally
            push!(rslt, remotecall(Core.eval, id_a, Main, :(ImagineWorker._output_analog_signals($sigsa, $writesz, $trigger_terminal, $rchan, $clock))))
        else
            set_device(dev, myid())
            push!(rslt, ImagineWorker._output_analog_signals(sigsa, writesz, trigger_terminal, rchan, clock))
        end
    ...
    if !isempty(sigsd)
        ...
        rchan = RemoteChannel(()->Channel{Int}(1))
        if !run_locally
            push!(rslt, remotecall(Core.eval, id_d, Main, :(ImagineWorker._output_digital_signals($sigsd, $writesz, $trigger_terminal, $rchan, $clock))))
        else
            set_device(dev, myid())
            push!(rslt, ImagineWorker._output_digital_signals(sigsd, writesz, trigger_terminal, rchan, clock))
        end
        push!(ready_chans, rchan)
    end    
    return (ready_chans, rslt)
end
output_signals(sig::ImagineSignal; trigger_terminal = "disabled", run_locally = false, clock = "") = output_signals([sig]; dev = DEFAULT_DEVICE, trigger_terminal = trigger_terminal, run_locally = run_locally, clock=clock)
...
function record_signals(base_name::AbstractString, sigs, nsamps::Integer; dev=DEFAULT_DEVICE, trigger_terminal = "disabled", run_locally=false, clock="")
    ...
    if !isempty(sigsa)
        ...
        if !run_locally
            push!(rslt, remotecall(Core.eval, id_a, Main, :(ImagineWorker._record_analog_signals($file_name, $sigsa, $nsamps, $readsz, $trigger_terminal, $rchan, $clock))))
        else
            set_device(dev, myid())
            push!(rslt, ImagineWorker._record_analog_signals(file_name, sigsa, nsamps, readsz, trigger_terminal, rchan, clock))
        end
        push!(ready_chans, rchan)
    end
    if !isempty(sigsd)
        ...
        if !run_locally
            push!(rslt, remotecall(Core.eval, id_d, Main, :(ImagineWorker._record_digital_signals($file_name, $sigsa, $nsamps, $readsz, $trigger_terminal, $rchan, $clock))))
        else
            set_device(dev, myid())
            push!(rslt, ImagineWorker._record_digital_signals(file_name, sigsd, nsamps, readsz, trigger_terminal, rchan, clock))
        end
        push!(ready_chans, rchan)
    end
    return (ready_chans, rslt)
end
record_signals(base_name::AbstractString, sig::ImagineSignal, nsamps::Integer; trigger_dest = "disabled", run_locally=false, clock="") = record_signals(base_name, [sig], nsamps; trigger_terminal = "disabled", run_locally = false, clock = clock)
